...
(defvar *textmate-gf-exclude* "/\\/\\.|(\\/|^)(\\.|dumps|vendor|ve|migrations|migration|fixtures|tmp|log|build)($|\\/)|(\\.xcodeproj|\\.nib|\\.framework|\\.app|\\.pbproj|\\.pbxproj|\\.xcode|\\.xcodeproj|\\.bundle|\\.pyc|\\.py~)$"
    ...
    (cond ((featurep 'aquamacs)
     ...
     (define-key map [(alt shift down)] 'textmate-column-down-with-select))
	  ...
	  (t ;; Any other version
	   ...
	   (define-key map [(control /)] 'comment-or-uncomment-region-or-line)
     (define-key map [(alt up)] 'textmate-column-up)
     (define-key map [(alt down)] 'textmate-column-down)
     (define-key map [(alt shift up)] 'textmate-column-up-with-select)
     (define-key map [(alt shift down)] 'textmate-column-down-with-select)))
...
A place is considered `tab-width' character columns."
  ...
  (let ((deactivate-mark nil)
    ...
    (indent-rigidly beg end (* (or arg 1) 4))))
